...
@router.put("/finishOrder/{id_order}", status_code=status.HTTP_200_OK)
async def finish_order(id_order:int, session:Session=Depends(get_session)):
    return OrderServices.finish_order(session, id_order)

@router.put("/cancelOrder/{id_order}", status_code=status.HTTP_200_OK)
async def cancel_order(id_order:int, session:Session=Depends(get_session)):
    return OrderServices.cancel_order(session, id_order)
